...
class NoisyDQN(nn.Module):
    def __init__(self, state_dim, n_actions):
        ...
        self.fc_val = NoisyLinear(256, 1)  # Value output as scalar
        self.fc_adv = NoisyLinear(256, n_actions)  # Advantage output as n_actions

    def forward(self, state):
        conv_out = self.conv(state)
        val = self.fc_val(conv_out)  # Shape: (batch_size, 1)
        adv = self.fc_adv(conv_out)  # Shape: (batch_size, n_actions)

        # Expand `val` to have the same number of actions as `adv`
        val = val.expand(-1, adv.size(1))  # Expanding val across actions dimension

        return val + adv - adv.mean(dim=1, keepdim=True)


# Custom Noisy Linear Layer
...
# Define the DQN Wrapper with Noisy DQN
class NoisyDQN_Agent(object):
    def __init__(self,
                 state_dim=37,
                 num_actions=25,
                 device='cpu',
                 ...
                 tau=0.1):
        self.device = device
        self.Q = NoisyDQN(state_dim, num_actions).to(device)
    ...
    def train(self, batches, epoch):
        ...
        for batch_idx in range(num_batch + 1):
            batch_uids = uids[batch_idx * batch_s: min((batch_idx + 1) * batch_s, len(uids))]  # Handle batch size
