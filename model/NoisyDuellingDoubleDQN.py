...
class NoisyLinear(nn.Module):
    def __init__(self, in_features, out_features, std_init=0.4):
        ...
        # Parameters for the noisy layers (weights and bias)
    ...
    def forward(self, input):
        ...
        weight_noise = torch.randn_like(self.weight_sigma) * self.weight_sigma
        bias_noise = torch.randn_like(self.bias_sigma) * self.bias_sigma
        
        # Apply the noisy weights and bias to the input
        noisy_weight = self.weight_mu + weight_noise
        noisy_bias = self.bias_mu + bias_noise
        
        return F.linear(input, noisy_weight, noisy_bias)

class NoisyDuellingDoubleDQN(nn.Module):
    def __init__(self, state_dim, n_actions):
        super(NoisyDuellingDoubleDQN, self).__init__()
    ...
    def forward(self, state):
        ...
        #print(conv_out.shape)  # Print the shape of conv_out
        val = self.fc_val(conv_out)
        adv = self.fc_adv(conv_out)
        return val + adv - adv.mean(dim=1, keepdim=True)


# Define the Noisy Duelling Double DQN Agent
class NoisyD3QN_Agent(object):
    def __init__(self,
                 state_dim=37,
                 num_actions=25,
                 device='cpu',
                 ...
                 tau=0.1):
        self.device = device
        self.Q = NoisyDuellingDoubleDQN(state_dim, num_actions).to(device)
    ...
    def compute_loss(self, batch, use_sofa=True):
        ...
        # Get the Q-values from the current state using the forward pass
        ...
        # Get the Q-values from the next state using the forward pass
        q_eval4nex = self.Q(next_state.clone().detach().float().to(self.device))
        max_eval_next = torch.argmax(q_eval4nex, dim=1)

        # Get the target Q-values from the target network (no gradients)
        with torch.no_grad():
            Q_dist_target = self.Q_target(next_state.clone().detach().float().to(self.device))

        # Get the Q-values corresponding to the next state and next action
        ...
        # Compute the target probabilities for the softmax-based weighting
        ...
        # Calculate the target Q-value using the weighted average of Q_dist_eval and Q_dist_tar
        Q_dist_star = (pro1 / (pro1 + pro2)) * Q_dist_eval + (pro2 / (pro1 + pro2)) * Q_dist_tar
        
        # Choose which Q-value to use based on SOFA score
        ...
        # Compute the target Q-values for the loss
        targetQ1 = reward + (gamma * Q_experi * end_multiplier)

        # Return the loss using Smooth L1 Loss (Huber loss)
