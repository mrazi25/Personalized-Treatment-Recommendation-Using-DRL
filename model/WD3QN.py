...
class WeightedDuellingDoubleDQN(nn.Module):
    def __init__(self, state_dim, n_actions):
        super(WeightedDuellingDoubleDQN, self).__init__()
...
class WD3QN_Agent(object):
    def __init__(self,
                 ...
                 tau=0.1):
        self.device = device
        self.Q = WeightedDuellingDoubleDQN(state_dim, num_actions).to(device)
        self.Q_target = copy.deepcopy(self.Q)
        self.tau = tau
        self.gamma = gamma
        self.num_actions = num_actions
        self.optimizer = optim.AdamW(self.Q.parameters(), lr=0.0001)
    ...
    def compute_loss(self, batch, use_sofa=True, beta_start=0.9):
        """
        Computes the loss using importance sampling weights.

        Parameters:
        - batch: Tuple containing state, next_state, action, next_action, reward, done, SOFA, prob.
        - use_sofa: Whether to condition Q-value updates on SOFA scores.
        - beta_start: Exponent for importance sampling weights.
        """
        ...
        prob = abs(reward)
        imp_weight = (1.0 / len_df) * (1.0 / prob)
        imp_weight = torch.pow(imp_weight, beta_start).to(self.device)
        ...
        td_error = targetQ1 - log_Q_dist_prediction1

        # Apply the importance sampling weights
        weighted_td_error = td_error * imp_weight

        # Return the loss using SmoothL1Loss
        return nn.SmoothL1Loss()(weighted_td_error, torch.zeros_like(weighted_td_error))
